...
\chapter{The acoustic scattering Problem in full space}\label{c1}
	...
	\section{Introduction}\label{c1se1}
		...
		\[
			c(x) = c_0 \text{ and } \gamma(x) = 0,\quad\forall |x|>\alpha,
		\]
		that means inhomogeneities of the medium lie inside some bounded region (inside a ball). Furthermore we assume, that outside this ball of radius $\alpha$, $\overline{B_\alpha(0)}$, there are sources that generate plane waves, that is functions of the type
		\[
	...
	\section{Theory for the direct Scattering Problem in $\mathbb{R}^3$}\label{c1se2}
		...
		\begin{enumerate}
			\item $\hat{\theta}\in\mathbb{R}^3$ with $|\hat{\theta}| = 1$ defines the \emph{indicent field}
				...
				\item $n\in L^\infty(\mathbb{R}^3),\re n\geq 0, \im n\geq 0$ and $n(x) = 1$ for all $x\in\mathbb{R}^3\setminus B_\alpha(0)$ and some $\alpha>0$
		\end{enumerate}
		We recall
		\[
		...
		\begin{problem}[Scattering Problem (S)]\label{c1se2prbS}
		...
		\begin{theorem}[Rellich's Lemma]\label{c1se2thm1}
			Let $u$ satisfy $\Delta u + \kappa^2 u$ for every $|x|>\alpha$. The following property 
			\[
				\lim_{R\to\infty} \int_{|x|=R} |u(x)|^2\,dS = 0
			\]
			implies, that $u(x) = 0$ for all $|x| > \alpha$.
		\end{theorem}
		\begin{remark}
			Vice versa, if the property in \cref{c1se2thm1} does not hold, then $u$ cannot vanish for all $|x| > \alpha$.
		\end{remark}
		\begin{proof}
		...
		\[
			||g||_{L^2(Q)} = \sum_{j\in\mathbb{Z}^3} |g_j|^2.
		\]
		...
		\begin{lemma}\label{c1se2thm2}
			Let $p\in\mathbb{R}^3, \alpha\in\mathbb{R}$ and $\hat e = \begin{bmatrix}1\\i\\0 \end{bmatrix}$. Then, for every $t>0$ and every $g\in L^2(Q)$ there exists a unique solution $w = w_t(g)\in H^1_{per}(Q)$ to the differential equation
			...
			\]
			for $\lambda_t\coloneqq 2t\hat e - i p$, $\mu_t = -(it + \alpha)$. It holds that
			\[
		...
		\begin{theorem}[Unique continuation principle]\label{c1se2thm3}
			Let $u\in H^1_{loc}(\mathbb{R}^3)$ solve $\Delta u + \kappa^2 n u = 0$, where $n\in L^\infty(\mathbb{R}^3)$ with $n(x) = 1$ for $|x| > \alpha$, and let $b\geq \alpha$, such that $u(x)=0$ for all $|x|\geq b$. Then we have $ u=0$ in $\mathbb{R}^3$.
		\end{theorem}
		\begin{remark}
